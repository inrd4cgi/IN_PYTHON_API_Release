...
namespace INS_INTERFACE
{
	...
	extern "C" INSDRIVER_EXPORT qint32 GetServerRootDirs(QList<INFileServerRoot> &root_dirs);

	//获取项目文件列表[projects]。
	//返回说明：1==获取成功;0==获取失败。
	extern "C" INSDRIVER_EXPORT qint32 GetProjectFiles(QList<INProjectFile>& projects);
	...
	//返回说明：1==创建成功;0==权限不足;
	extern "C" INSDRIVER_EXPORT qint32 GetFolder(INFolderBase &folderbase, QMap<qint32, INFileBase> &files);

	//获取指定文件的历史版本列表。要求[filebase]中有文件id
	//返回说明：1==获取成功;0==获取失败;
	extern "C" INSDRIVER_EXPORT qint32 GetFileHistory(INFileBase &filebase, QList<INFileHistory> &file_histories);
	...
	extern "C" INSDRIVER_EXPORT qint32 RenameFolder(INFolderBase & folder, QMap<qint32, INFileBase> &files);


	//将文件[files]放入回收站。可以是多个，但要求是在同一个文件夹下。[files]中条目要求有文件id，文件夹id，项目id。
	//返回说明：1==成功放入回收站;0==权限不足;-1==数据出错。
	extern "C" INSDRIVER_EXPORT qint32 SendFilesToRecycleBin(const QList<INFileBase>& files);

	//将文件夹[folders]放入回收站。可以是多个，但要求是在同一个父级文件夹下。[folders]中条目要求有文件夹id，项目id。
	//返回说明：1==成功放入回收站;0==权限不足;-1==数据出错。
	extern "C" INSDRIVER_EXPORT qint32 SendFoldersToRecycleBin(const QList<INFolderBase>& folders);
	...
	extern "C" INSDRIVER_EXPORT qint32 RestoreRecycleBin(const qint32& project_id, const QList<qint32>& items_id, QList<INFileBase>& files, qint32& code);

	//回复服务器要替换的文件列表.[code]为调用RestoreRecycleBin时产生的业务关联码。[exit]为是否放弃恢复。
	//[files]为要replace掉的文件,即删除掉当前文件，选用回收站中的文件。
	//返回说明：1==恢复成功;0==恢复失败。
	extern "C" INSDRIVER_EXPORT qint32 ReplaceFiles(const qint32& code, const qint32& exit, const QList<INFileBase>& files);
