...
namespace INS
{
	...
	************************************************************************/
	struct Role {
		...
		Serialization(Role, roleId, roleName, roleType, parentId, entryId, subEntryId,
	...
	struct INQProjectPermissionVO {
		...
		QMap<qint32, QString> filePaths;//文件操作对应的文件名称和路径的列表[key: id,value: path]

		Serialization(INQProjectPermissionVO, projectId, templateId, templateName, grade, isDefault, permissionMap, filePaths)
	};
	...
	struct PersonPermissionVO {
		...
		Serialization(PersonPermissionVO, personId, roles, masterPermissions, detailPermissions)
	};
	...
	struct INQAdministrationPermissionVO {
		...
		Serialization(INQAdministrationPermissionVO, personId, roleId, roleName,
	...
	struct INQPersonTemplateVO {
		...
		Serialization(INQPersonTemplateVO, projectId, teamId, templateId, personId,
