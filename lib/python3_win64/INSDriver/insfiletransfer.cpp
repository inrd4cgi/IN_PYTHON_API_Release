...
namespace INS
{
	...
	*************************************************************************************************/
	INSFileTransfer::INSFileTransfer(const INFileBase & file, const QString & rootDir, const QString& absolutePath)
	...
	*/
	...
	void INSFileTransfer::WaitAppServerResponse()
	{
		...
		mp_in->device()->seek(0);
		*mp_in >> m_responFromAppSever >> m_fileRepoInfo;

		m_file = m_fileRepoInfo.infoInClient;
	 ...
	 * \return
	 */
	INSUpLoad::INSUpLoad(const INFileBase& file, const QString& rootDir, const QString& absolutePath)
	...
	bool INSUpLoad::Start()
	{
		...
		if (m_file.folder_id > 0)
			transferInfo.insert(QString("folderId"), m_file.folder_id);
		else
			transferInfo.insert(QString("directory"), m_file.directory);

		assert(m_file.id > 0 || !m_file.name.isEmpty());
		if (m_file.id > 0)
			transferInfo.insert(QString("fileId"), m_file.id);
		else
			transferInfo.insert(QString("name"), m_file.name);

		m_file.md5 = GetFileMd5(&m_fileHandle);
		transferInfo.insert(QString("checkCode"), m_file.md5);

		//添加延申的数据，方便传输其他类型的文件拓展
		AddExtendData(transferInfo);

		qDebug() << "md5 " << m_file.md5;

		*mp_out << qint32(304) << m_request_id << transferInfo;
	...
	void INSUpLoad::BeginFileTransferStep()
	{
		...
		{
			//初次上传，初始化记录文件
			m_fileRecorder->InitRecordFileByLocalFile(m_transferType, m_file.md5);
	...
	**************************************************************************************************/
	INSDownLoad::INSDownLoad(const INFileBase& file, const QString& rootDir)
	...
	*************************************************************************************************/
	bool INSDownLoad::Start()
	{
		...
		if (m_file.version > 0)
			transferInfo.insert(QString("version"), m_file.version);
	...
	void INSDownLoad::BeginFileTransferStep()
	{
		if (m_file.directory.isEmpty())
			m_file.directory = m_fileRepoInfo.infoInClient.directory;
		if (m_file.name.isEmpty())
			m_file.name = m_fileRepoInfo.infoInClient.name;
		InitFileHandle(true);
		...
		if (!m_fileRecorder->isRecordFileExist())
			m_fileRecorder->InitRecordFile(m_file.size, m_file.datetime_modify, m_transferType, m_file.md5);
	...
	//INSCheckIn
	//***************************************************************************************************
	INSCheckIn::INSCheckIn(const INFileBase & file, const QString & rootDir, const QString& absolutePath)
	...
	INSCheckOut::INSCheckOut(const INFileBase & file, const QString & rootDir)
	...
	void INSCheckOut::SendCheckOutReqToAppServer()
	{
		...
		varMap.insert("fileId", m_file.id);

		m_senddata.clear();
		mp_out->device()->seek(0);
		*mp_out << qint32(304) << m_request_id << varMap;
	...
	//INSUploadTinyFile
	INSUploadTinyFile::INSUploadTinyFile(const INFileBase & file, const QString & rootDir, const QString & absolutePath)
		:INSFileTransfer(file, rootDir, absolutePath)
	{
		m_transferType = 0x00010000;
		m_fileHandleOpenMode = QIODevice::ReadOnly;
	...
	///INSUploadUserAvatar
	INSUpLoadUserAvatar::INSUpLoadUserAvatar(const INFileBase &file, const QString &absolutePath)
	...
	//*************************************************************************************************
	INSUploadLibraryFile::INSUploadLibraryFile(const INFileBase & fileBase, const QString & absolutePath)
	...
	void InitUploadInfoOperator::RunRequest()
	{
		...
		{ QString("checkcode"),m_transferObj->m_file.md5 } };
	...
	void DownloadFileOperator::SaveFileData(QByteArray &byteArray)
	{
		...
		{
			m_transferObj->m_fileHandle.setFileTime(m_transferObj->m_file.datetime_modify, QFileDevice::FileModificationTime);
			m_transferObj->m_fileHandle.setFileTime(m_transferObj->m_file.datetime_create, QFileDevice::FileBirthTime);
	...
	void UploadTinyFileOperator::RunRequest()
	{
		...
		reqParam += QString("checkcode=%1").arg(m_transferObj->m_file.md5);
