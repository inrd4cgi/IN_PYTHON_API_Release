...
namespace INS {
    ...
    **************************************************************************************************/
    ...
    INSLoginConflictListener *INSLoginConflictListener::GetInstance() {
	    static INSLoginConflictListener instance;
        return &instance;
    }
    ...
    void INSLoginConflictListener::Process(const QByteArray &byteArray) {
        ...
        *mp_in >> jsonStr;
        ...
        //同时发给python那边。
        INSPythonNotifier::GetInstance()->LogOff(msg);
    }

    INSPythonNotifier::INSPythonNotifier(QObject *parent) : QObject(parent) {
        INSLoginConflictListener::GetInstance();
        INSNetwork * network = INSNetwork::GetInstance();

        connect(network, SIGNAL(SigAppServerSocketStatus(bool)), this, SLOT(SlotAppServerSocketStatus(bool)));
    }

    void INSPythonNotifier::LogOff(QString msg) {
        emit EventLogOff(msg);
    }

    void INSPythonNotifier::Disconnected() {
        emit EventDisconnected();
    }

    INSPythonNotifier *INSPythonNotifier::GetInstance() {
        static INSPythonNotifier instance;
        return &instance;
    }

    void INSPythonNotifier::SlotAppServerSocketStatus(bool status) {
        if (!status) {
            this->Disconnected();
        }
    }
