...
namespace INS_INTERFACE
{
	...
	INSDRIVER_EXPORT MessageInfo BatchMakeTags(const QList<TagParam>& tagParams) {
		INSCommonRequest<int, QList<TagParam>> commonRequest(606, tagParams);
		commonRequest.WaitForFinished();
		return commonRequest.m_return_value;
	}
    ...
    {
        INSCommonRequest<qint32, qint32, QList<qint32 >> commonRequest(580, prjId, std::move(personList));
        commonRequest.WaitForFinished();
        return commonRequest.m_return_value;
    }
